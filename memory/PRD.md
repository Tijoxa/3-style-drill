...
- **Timer start refinement** [2026-06]: `buildCase(startImmediately)` — first load / mode switch / skip / reset wait for the first move (with a **30s auto-start timeout** via `noMoveTimeoutRef`, cancelled on first move or next case); after finishing a pair (`onSuccess` -> `buildCase(true)`) the timer runs immediately without waiting for a move. Verified fresh-load stays 0, post-solve counts immediately.
